        ...
        <div class="modal-content">
                    ...
                    <div class="form-group" >
                        <label class=" control-label" for="example-chosen">Type pièce<span class="text-danger">*</span></label>
                            <select  name="type_document" class="form-control  type_document" data-placeholder="selection le type de document" style="width: 100%;">
                                <option></option>
                                <option value="declaration">Déclaration sur l'honneur</option>
            ...
            <div class="row">
                <div class="form-group{{ $errors->has('piece_jointe') ? ' has-error' : '' }}">
                    <label class=" control-label" for="piece_contractuelle">Joindre la pièce<span class="text-success">*</span></label>
...
form.children("div").steps({
    ...
    onFinished: function (event, currentIndex)
    {   if(!$('#confirmation_pm').is(':checked') || (!$('.signature').val()) ){
        //alert('veuillez signer');
        $('.signature_resume').show();
    }
    else{
        //alert('esle');
        $('.signature_resume').hide();
...
<script>
    function chiffre_daffaire(){
        var val=$('#chiffre_daffaire_prev_pm').val();
        if(val<1000000){
            alert('ok');
        }
    }
